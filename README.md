...
The web demo is **fully live — nothing is simulated**: Agent A is a real Gemini 3.5 Flash call that fixes a real bug in a real temp git repository; Agent B is a real Gemini call deciding its own next action from the handoff it receives; tool calls execute real `git`/`pytest` subprocesses whose output streams into terminal panes; and every proposal is gated by the trained adjudicator against the agent's own live justification text, with Gemma 4 E2B writing the replan messages on-device. The left pipeline hits a real `git checkout` error on the missing branch; the right pipeline gets gated, re-plans, and real tests pass. A second tab renders the **task graph live** (node statuses + gate log) as agents act. The "kill network" toggle swaps Agent B itself to local Gemma 4 — decisions, gate, and explanations all on-device.
